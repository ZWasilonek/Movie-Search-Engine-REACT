...
const loadMovies = (title, page) => async dispatch => {
  ...
  await fetch(`http://www.omdbapi.com/?s=${title}&page=${page}&type=movie&apikey=${OMDb_API_KEY}`)
...
const loadMovieDetails = (movieID) => async dispatch => {
  dispatch(startFetching());
  await fetch(`http://www.omdbapi.com/?i=${movieID}&plot=full&type=movie&apikey=${OMDb_API_KEY}`)
    .then(response => response.json())
    .then(data => typeof data !== "undefined" && data.Response === "False" ?
      dispatch(setError(data)) :
